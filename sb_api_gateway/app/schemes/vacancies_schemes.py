...
class SubcategoryScheme(CatBaseScheme):
    category_id: int = Field(..., alias="ID категории")
...
class VacancyScheme(BaseModel):
    ...
    salary_from: Optional[float] = Field(None, ge=0, description="Зарплата от")
    salary_to: Optional[float] = Field(None, ge=0, description="Зарплата до")
