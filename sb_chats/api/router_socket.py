...
from api.dao import DBMessages
from api.security import EncryptionService

db = DBMessages()

router = APIRouter(prefix="/ws/chat", tags=['Чаты'])
        ...
        if room_id in self.active_connections:
            for user_id, connection in self.active_connections[room_id].items():
                ...
                await db.create_message(
                    chat_id=room_id,
                    content=message_with_class['text'],
                    sender_id=sender_id,
                    message_type='text',
                    is_read=False
                )
...
async def websocket_endpoint(websocket: WebSocket, room_id: int, user_id: int, username: str):
    ...
    messages_history = await db.get_messages_by_chat_id(chat_id=room_id)
    for messages in messages_history:
        for msg in messages['chat']['messages']:
            await manager.broadcast(message=msg['content'],
